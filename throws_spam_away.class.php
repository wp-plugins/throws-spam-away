...
class ThrowsSpamAway {
	...
	function comment_form_empty_param_field() {
		// 空パラメータフィールド作成
		$empty_param_field_flg = get_option( 'tsa_empty_param_field_flg', "1");
		if ( $empty_param_field_flg == "1" ) {
			echo '<p id="throwsSpamAway_empty_param" style="display:none;">メールアドレス（もう一度）<span class="required">*</span><input type="text" name="tsa_email_param_field___" id="tsa_email_param_field___" size="30" value=""/>
	</p>';
		}
		return TRUE;
	}
	...
	function validation( $comment, $author ) {
		...
		$tsa_empty_param_field_flg = get_option( 'tsa_empty_param_field_flg', "1" );
		if ( $tsa_empty_param_field_flg == "1") {
			if ( !empty( $_POST['tsa_email_param_field___'] ) ) { // このフィールドにリクエストパラメータが入る場合はスパム判定
				$error_type = "empty_param_field";
				return FALSE;
			}
		}
		// シングルバイトだけならエラー
...
<div class="wrap">
	...
	<form method="post" action="options.php">
		...
		<table class="form-table">
			<tr valign="top">
				<th scope="row">人の目には見えないダミーの入力項目を作成し、そこに入力があれば無視対象とする<br />（すべての項目を埋めようとするマシン投稿に有効です）</th>
				<td><?php
				$chk_1 = "";
				$chk_2 = "";
				if ( get_option( 'tsa_empty_param_field_flg', "1") == "2" ) {
					$chk_2 = " checked=\"checked\"";
				} else {
					$chk_1 = " checked=\"checked\"";
				}
			?><label><input type="radio" name="tsa_empty_param_field_flg" value="1"<?php echo $chk_1;?> />&nbsp;する</label>&nbsp;
			  <label><input type="radio" name="tsa_empty_param_field_flg" value="2"<?php echo $chk_2;?> />&nbsp;しない</label>
			  	</td>
			</tr>
			<tr valign="top">
